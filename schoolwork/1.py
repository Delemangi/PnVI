...
def main():
    global FPSCLOCK, DISPLAYSURF, fails
    pygame.init()
...
def revealBoxesAnimation(board, boxesToReveal):
    # сменети се насоките за покривање и откривање
...
# додаден е трет параметар во функцијата за боја
def drawBoard(board, revealed, box_color=None):
    ...
    for boxx in range(BOARDWIDTH):
        for boxy in range(BOARDHEIGHT):
            ...
            if not revealed[boxx][boxy]:
                ...
                # услов за проверка дали играчот тепал
                if hasWon(revealed):
                    # третиот параметар се користи тука како боја на квадратот
                    pygame.draw.rect(
                        DISPLAYSURF,
                        box_color if box_color is not None else HIGHLIGHTCOLOR,
...
def gameWonAnimation(board):
    ...
    for i in range(13):
        ...
        drawBoard(board, coveredBoxes, color3)  # нов параметар на функцијата
        pygame.display.update()
