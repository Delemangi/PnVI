...
import random
import sys
import time  # додавање на нова библиотека
...
def runLevel(levels, levelNum):
    ...
    # додадени се променливите за почетокот на времето и изминатото време
    startTime = time.time()
    elapsedTime = 0
    ...
    while True:  # main game loop
        ...
        for event in pygame.event.get():  # event handling loop
            ...
            # калкулирање на изминатото време
        ...
        # рендерирање на тековното време
        timeSurf = BASICFONT.render(
            "Time: {:.2f} seconds".format(elapsedTime), 1, TEXTCOLOR
        )
        timeRect = timeSurf.get_rect()
        timeRect.bottomleft = (20, WINHEIGHT - 50)
...
def decorateMap(mapObj, startxy):
    ...
    # додавање на камења на нивото
    for _ in range(5):
        while True:
            x = random.randint(0, len(mapObj) - 1)
            y = random.randint(0, len(mapObj[x]) - 1)
            if mapObj[x][y] == " ":
                mapObj[x][y] = "rock"
                break
...
def drawMap(mapObj, gameStateObj, goals):
    ...
    for x in range(len(mapObj)):
        for y in range(len(mapObj[x])):
            ...
            # рендеирање на камењата на нивото
            if mapObj[x][y] == "rock":
...
# приказ на нов екран за победа
def displayVictoryMessage():
    victorySurf = BASICFONT.render("Congratulations! You won!", True, WHITE)
    victoryRect = victorySurf.get_rect()
    victoryRect.center = (HALF_WINWIDTH, HALF_WINHEIGHT)
    DISPLAYSURF.blit(victorySurf, victoryRect)
    pygame.display.update()
